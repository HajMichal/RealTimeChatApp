...
function generateAccessToken( user: newUser ){
    return jwt.sign( user, process.env.ACCESS_TOKEN_SECRET, { expiresIn: '5000s' } )
}

function createAccessToken( user: newUser, res: Response) {
    const accessToken = generateAccessToken(user)
    const refreshToken = jwt.sign( user, process.env.REFRESH_TOKEN_SECRET, { expiresIn: '10000s' }  )
